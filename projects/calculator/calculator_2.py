...
class CalcFrame(wx.Frame):
    ...
    def handler(self, button, label):
        if self.is_num(self.equation):
    ...
    def on_calc(self, event):
        ...
        if self.flag:
            if self.is_num(label):
                self.equation = ""
            self.flag = False
        if label == '.' and '.' in self.equation:
            return
        self.equation += label
        self.textprint.SetValue(self.equation)
